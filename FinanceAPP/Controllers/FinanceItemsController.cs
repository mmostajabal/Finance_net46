...
namespace FinanceAPP.Controllers
{
    public class FinanceItemsController : Controller
    {
        ...
        public FinanceItemsController(IUploadFile uploadFileSrv, IPartner partnerSrv, IFinancialItem financialItemSrv, ITransferExcel2List<FinancialItemsDTO> transferExcel2ListSrv, IPartnerFinancialItems partnerFinancialItemsSrv)
        {
            //, IPartnerFinancialItems partnerFinancialItemsSrv
        ...
        public ActionResult Index(HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                List<FinancialItemsDTO> financialItems = _transferExcel2ListSrv.TransferExcel2List(Path.Combine(uploadFolder, fileName));
                numberRecords = financialItems.Count;
                message = "Number records : " + numberRecords.ToString();

                financialItems = _partnerFinancialItemsSrv.RemoveFinancialItemHasWrongPatnerId(financialItems);

                message += " Number of correct information : " + financialItems.Count.ToString();
                message += " Number of wrong information : " + (numberRecords - financialItems.Count).ToString();

                Log.Information(message);

                foreach(FinancialItemsDTO financialItem in financialItems)
                {
                    financialItem.FinancialId = Guid.NewGuid();
                }

                GlobalVariables.FINANCIAL_ITEMS_LIST.AddRange(financialItems);
        ...
        /// <returns></returns>
        public ActionResult FinancialItem()
        {
        ...
        public ActionResult Create()
        {
            ...
            {
                Log.Error($" Financial item Create {ex.ToString()}");
            }
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(FinancialItemsDTO financialItems)
        {
            try
            {
                TempData["header"] = "Create Financial Item";
                ...
                if (ModelState.IsValid)
                {
                    ...
                    ModelState.SetModelValue("Amount", new ValueProviderResult(string.Empty, string.Empty, CultureInfo.InvariantCulture));
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Edit(Guid financialId)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(FinancialItemsDTO financialItems)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (_financialItemSrv.Update(financialItems))
                    {
                        ...
                        TempData["success"] = " Financial item updated successfully";
                    }
                    else
                    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Delete(Guid financialId)
        {
            FinancialItemsDTO financialItem = new FinancialItemsDTO();
            try
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(FinancialItemsDTO financialItems)
        {
            try
            {
                TempData["header"] = "Edit Financial Item";
                ...
                if (_financialItemSrv.Delete(financialItems.FinancialId))
                {
                    ...
                    TempData["success"] = "Financial Item deleted successfully";
                }
                else
                {
                    Log.Warning($"Could not find financial item {financialItems.FinancialId} by ");
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Details(Guid financialId)
        {
            try
            {
                FinancialItemsDTO financialItem = new FinancialItemsDTO();
